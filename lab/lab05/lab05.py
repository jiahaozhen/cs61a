...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sum([[deck[k], deck[k+len(deck)//2]] for k in range(len(deck)//2)], [])
...
def add_trees(t1, t2):
    ...
    return tree(label_new, [add_trees(t1_b, t2_b) for t1_b, t2_b in zip_longest(branches(t1), branches(t2))])
