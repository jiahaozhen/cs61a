...
def subseqs(s):
    ...
    if len(s) == 0:
        ...
        without_first_element = subseqs(s[1:])
        return without_first_element + insert_into_all(s[0], without_first_element)
...
class Keyboard:
    ...
    def press(self, info):
        ...
        if info in self.buttons:
            ...
            return_string = self.buttons[info].key
...
def make_advanced_counter_maker():
    ...
    """
    global_count = 0
    def advance_count_maker(count=0):
        nonlocal global_count
        def advance_count(info):
...
def prune_small(t, n):
    ...
    while len(t.branches) > n:
        ...
        t.branches = [branch for branch in t.branches if branch.label != largest.label]
    for b in t.branches:
        prune_small(b, n)
