...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            max = score0-last_score
            if max > running_high:
                print(max, 'point(s)! The most yet for Player 0')
            else:
                max = running_high
            return announce_highest(who, score0, max)
        else:
            max = score1-last_score
            if max > running_high:
                print(max, 'point(s)! The most yet for Player 1')
            else:
                max = running_high
            return announce_highest(who, score1, max)
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_rolls = 0
    max_sum = 0
    while temp_num_rolls <= 10:
        temp_max = make_averaged(roll_dice, trials_count)(temp_num_rolls, dice)
        if temp_max > max_sum:
            max_rolls = temp_num_rolls
            max_sum = temp_max
        temp_num_rolls += 1
    return max_rolls
    # END PROBLEM 9
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    if cutoff <= free_bacon(opponent_score):
        return 0
    else:
        return num_rolls
