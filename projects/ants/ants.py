...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        pos = 0
        present = self.place
        while pos < self.min_range and present != beehive:
            present = present.entrance
            pos += 1
        while present != beehive:
            if pos > self.max_range:
                return None
            elif len(present.bees) == 0:
                present = present.entrance
                pos += 1
            else:
                break
        if present == beehive:
            return None
        return rANTdom_else_none(present.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.appearance += 1
        self.is_True_Queen = QueenAnt.appearance == 1
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        present = self.place.exit
        while present != None:
            if present.ant != None:
                if not hasattr(present.ant, 'is_doubled'):
                    present.ant.damage *= 2
                    present.ant.is_doubled = True
            present = present.exit
        # END Problem EC
